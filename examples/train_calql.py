...
def train(
    ...
    start_step = (
        int(
            os.path.basename(
                checkpoints.latest_checkpoint(os.path.abspath(FLAGS.calql_checkpoint_path))
            )[11:]
        )
        + 1
        if FLAGS.calql_checkpoint_path and glob.glob(os.path.join(FLAGS.calql_checkpoint_path, "checkpoint_*"))
        else 0
    )
...
def main(_):
    ...
    if not eval_mode:
        ...
        wandb_logger = make_wandb_logger(
            project="hil-serl",
            description=f"calql_alpha_{calql_agent.config['cql_alpha']}_reward_bias_{FLAGS.reward_bias}_discount_{config.discount}_{FLAGS.exp_name}",
            debug=FLAGS.debug,
