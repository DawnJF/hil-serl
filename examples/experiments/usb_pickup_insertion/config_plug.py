...
class TrainConfig(DefaultTrainingConfig):
    ...
    setup_mode = "calql-single-arm-learned-gripper"
    # setup_mode = "iql"
    # setup_mode = "calql"
...
def test_images():
    ...
    Image.fromarray(rgb.squeeze(0)).save("outputs/test_rgb/a_rgb.png")
    Image.fromarray(wrist.squeeze(0)).save("outputs/test_wrist/a_wrist.png")
    Image.fromarray(scene.squeeze(0)).save("outputs/test_scene/a_scene.png")
...
def test_reward_model():
    ...
    while True:
        ...
        os.makedirs(f"outputs/test_wrist", exist_ok=True)
        os.makedirs(f"outputs/test_scene", exist_ok=True)
        Image.fromarray(rgb.squeeze(0)).save(f"outputs/test_rgb/a_rgb_{i}.png")
        Image.fromarray(wrist.squeeze(0)).save(f"outputs/test_wrist/a_wrist_{i}.png")
        Image.fromarray(scene.squeeze(0)).save(f"outputs/test_scene/a_scene_{i}.png")
        if done:
            env.reset()
        i += 1
